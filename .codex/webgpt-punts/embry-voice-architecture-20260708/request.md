...
Stress test the Embry voice/chat system to identify concrete failures across:

1. RealtimeSTT ingress
2. speaker identity / diarization
3. memory/Tau routing
4. Chatterbox speech
5. shared Chat UX sync
6. Embry orb sync
7. replay
8. interruption / barge-in
9. research-backed answers through Brave Search
10. voice delivery tone/emotion from memory intent

The human wants approximately 200 labeled session tests with difficulty, pass/fail, receipt paths, and logs. Tests must be real/non-mocked where the route claims live behavior.

## Current Local Evidence

Repository: `/home/graham/workspace/experiments/chatterbox`
...
The 200-session matrix has 10 route families across 5 difficulties:

1. `memory.sparta_qra`
2. `memory.persona_memory`
3. `memory.persona_memory.fail_closed`
4. `brave-search.source_receipt`
5. `tau.agent_handoff`
6. `ux-lab.shared_chat`
7. `chatterbox.turn_control`
8. `memory.speaker.resolve`
9. `realtimestt.factory_capture`
10. `memory.intent.voice_delivery`
