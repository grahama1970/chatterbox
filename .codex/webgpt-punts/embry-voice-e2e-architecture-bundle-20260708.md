...
origin main: grahama1970/chatterbox
latest pushed commit: 2727b223fb4ab57f146fd33ab48b415d0f373911
```

Relevant current evidence artifacts:
...
But this is misleading for the final objective because many `passed` rows are text/memory answerability only.
...
```text
/tmp/chatterbox-fork-agent-out/embry-intelligence-stress/20260708T080421Z-medium-sparta-qra-after-scope-fix/receipt.json
```
...
Fresh Jabra source-62 physical mic receipt:

```text
/tmp/chatterbox-fork-agent-out/voice-chat-e2e/20260708T101852Z-factory-jabra-physical-source62/S06-factory-noise/rung8-loopback-listener.json
```
...
```text
mocked: false
capture kind: physical_microphone
captured RMS: 229
played source RMS: 542
RealtimeSTT transcript: "Thank you very much."
...
```text
/tmp/chatterbox-fork-agent-out/speaker-segment-evidence/20260708T102516Z-source62-speaker-segment-venv/speaker-segment-evidence.json
```
...
failed gates: min_primary_ratio, mean_primary_margin
```

This suggests the captured physical-mic waveform does not resemble the Horus enrollment to the current verifier.
...
```text
/tmp/chatterbox-listener-venv/bin/python import pyannote.audio
fails: AttributeError partially initialized module 'torchvision' has no attribute 'extension'
```

Also, `~/.zshrc` defines `HF_TOKEN` but does not export it, so child processes do not see the token:
...
- `KoljaB/RealtimeSTT` GitHub and PyPI snippets say to set `use_microphone=False` when audio comes from a file, stream, websocket, or another process, and feed 16-bit mono PCM chunks at 16 kHz or pass original sample rate so RealtimeSTT can resample.
...
```text
/tmp/chatterbox-listener-venv import pyannote.audio fails because torchvision is broken against installed torch.
HF_TOKEN is defined in ~/.zshrc but not exported, so child processes do not see it.
```
...
- Chrome MEI significant playback events require more than seven seconds of playback; stopping/closing/reloading before seven seconds can remove autoplay ability.
